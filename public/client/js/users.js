...
socket.on("SERVER_RETURN_INFO_ACCEPT_FRIEND", (data)=>{
  ...
  const idB = dataUsersAccept.getAttribute("data-users-accept")
  if(idB == data.idB){
    ...
    const btnRefuseFriend = newBoxUser.querySelector("[btn-refuse-friend]")
      btnRefuseFriend.addEventListener("click",()=>{
        ...
        const idB = btnRefuseFriend.getAttribute("btn-refuse-friend")
        socket.emit("CLIENT_REFUSE_FRIEND", idB)
      })
    ...
    btnAcceptFriend.addEventListener("click",()=>{
      ...
      const idB = btnAcceptFriend.getAttribute("btn-accept-friend")
      socket.emit("CLIENT_ACCEPT_FRIEND", idB)
    })
...
// End Trang lời mời kết bạn
  ...
  if(dataUsersNotFriend){
    const idB = dataUsersNotFriend.getAttribute("data-users-not-friend")
    if(idB == data.idB){
...
socket.on("SERVER_RETURN_USER_ID_CANCEL_FRIEND", (data)=>{
  ...
  if(idB == data.idB){
    // Xóa A khỏi danh sách của B
    const boxUserRemove = dataUsersAccept.querySelector(`[user-id="${data.idA}"]`)
    if(boxUserRemove){
